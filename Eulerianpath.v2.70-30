...
int main() {
	...
	while (A3.size() != v1) {
		...
		while (exit != -5) {
			...
			if ((find(A2[lastelement].begin(), A2[lastelement].end(), A1[lastelement][i1]) == A2[lastelement].end()) && (find(A3.begin(), A3.end(), A1[lastelement][i1]) == A3.end())) //ïðîâåðêà íà ïðèñóòñòâèå ýëåìåíòà
			{
				i2 = i1;
